class Solution {
public:
    // Intuition: We solve it same as recursion on subsequence problem where we find subsequence sum of given target_sum
    // just a tweak here we have to find same target multiple times so we simply find one and keep marking the visited
    // index so that for next time we don't consider the same index
